...
using CustomRadioStations.Model;
using Logging;
...
namespace CustomRadioStations
{
    public partial class MainScript : BaseScript
    {
        ...
        Control ControlSkipTrack;
        Control ControlVolumeDown;
        Control ControlVolumeUp;
        ...
        bool loaded;
        private UserWheelCats _userWheelCats;
        private long _iconsRadioTxd;
        ...
        public MainScript()
        {
            ...
            var path = Constants.MAIN_PATH + "categories.json";
            Logger.Log("looking for categories.json at " + path);
            var res = API.LoadResourceFile(API.GetCurrentResourceName(), path);
            if (string.IsNullOrWhiteSpace(res))
                throw new NullReferenceException("NO CATEGORIES.json found in radios directory");

            Logger.Log(res.Substring(1));

            _userWheelCats = JsonConvert.DeserializeObject<UserWheelCats>(res.Substring(1));

            Config.LoadINI();

            //just uses debug.writeline (cfx)
            //Logger.Init();

            //already commented, done later
            //SetupRadio();            

            //TODO: keyup 
            //KeyUp += OnKeyUp;

            API.RegisterNuiCallbackType(nameof(setCurPos)); // register the type
            API.RegisterNuiCallbackType(nameof(setSongLoaded)); // register the type
        }
        ...
        async Task HandleGamePauseAsync()
        {
            if (Game.IsControlJustPressed(2, ControlHelper.UsingGamepad() ? CitizenFX.Core.Control.FrontendPause : CitizenFX.Core.Control.FrontendPauseAlternate))
            {
                if (RadioStation.CurrentPlaying != null)
                {
        ...
        void HandleRadioWheelExtraControls()
        {
            if (WheelVars.CurrentRadioWheel.Visible)
            {
                if (RadioStation.CurrentPlaying != null)
                {
                    ...
                    ControlNextWheel = ControlHelper.UsingGamepad() ? CitizenFX.Core.Control.VehicleAccelerate : CitizenFX.Core.Control.WeaponWheelPrev;
                    ControlPrevWheel = ControlHelper.UsingGamepad() ? CitizenFX.Core.Control.VehicleBrake : CitizenFX.Core.Control.WeaponWheelNext;

                    if (Config.DisplayHelpText)
                    {
                        CitizenFX.Core.UI.Screen.DisplayHelpTextThisFrame(
                            ...
                            " : Volume: " +
                            Math.Round(SoundFile.SoundVolume * 100, 0) + "%\n" +
            ...
            if (RadioStation.CurrentPlaying != null)
            {
                Game.DisableControlThisFrame(2, CitizenFX.Core.Control.VehicleNextRadio);
                Game.DisableControlThisFrame(2, CitizenFX.Core.Control.VehicleNextRadioTrack);
                Game.DisableControlThisFrame(2, CitizenFX.Core.Control.VehiclePrevRadio);
                Game.DisableControlThisFrame(2, CitizenFX.Core.Control.VehiclePrevRadioTrack);
        ...
        void HandleRadioWheelToggle()
        {
            ...
            {
                CitizenFX.Core.Debug.WriteLine(ex.ToString());
            }
        }
        ...
        async void OnPlayerEnteredVehicle(Vehicle veh)
        {
        ...
        async Task OnTickAsync()
        {
            if (!loaded)
            {
                ...
                {
                    if (Config.DisplayHelpText)
                        CitizenFX.Core.UI.Screen.ShowSubtitle("Loading Custom Radios...");
                    ...
                    if (Config.DisplayHelpText)
                        CitizenFX.Core.UI.Screen.ShowSubtitle("Custom Radios Loaded");
                    ...
                    API.RequestStreamedTextureDict("radio_icons", false);
                }
            ...
            //    CitizenFX.Core.UI.Screen.ShowSubtitle("Custom Radio INIs reloaded:\n- settings.ini\n- station.ini files\n- Scanned for tracklists");
            //    await Delay(150);
            //}

            if (VanillaOrCustomRadioWheelIsVisible())
            {
                if (ControlHelper.UsingGamepad() && Game.IsControlJustPressed(2, Config.GP_Toggle))
                {
                    HandleRadioWheelToggle();
                }
                else if (!ControlHelper.UsingGamepad() && Game.IsControlJustPressed(2, Config.KB_Toggle))
                {
                    if (!loaded)
                    {
                        CitizenFX.Core.UI.Screen.ShowSubtitle("Custom Radio not loaded yet, please try again later!");
                        return;
                    }
            ...
            if (Game.IsControlJustReleased(2, CitizenFX.Core.Control.VehicleRadioWheel))
            {
                if (WheelVars.CurrentRadioWheel.Visible)
                {
                    WheelVars.CurrentRadioWheel.Visible = false;
        ...
        void SetupRadio()
        {
            ...
            //TODO: change this so it pulls from json file
            //string[] wheelDirectories = Directory.EnumerateDirectories(Constants.MAIN_PATH, "*", SearchOption.TopDirectoryOnly)?.ToArray();

            var res = API.LoadResourceFile(API.GetCurrentResourceName(), Constants.MAIN_PATH + "categories.json");
            if (string.IsNullOrWhiteSpace(res))
                throw new NullReferenceException("NO CATEGORIES.json found in radios directory");

            _iconsRadioTxd = API.CreateRuntimeTxd("radio_icons");

            foreach (var wheelDir in _userWheelCats.WheelCats)
            {
                //radios/catpath
                var catPath = $"{Constants.MAIN_PATH}{wheelDir.name}";

                Logger.Log("Loading " + catPath);
                Logger.Log("Checking if " + wheelDir.name + "\\settings.ini exists");

                //shouldnt need this? already loaded before
                //var wheelIni = Config.LoadWheelINI(wheelDir);

                // Create wheel obj
                Wheel radioWheel = new Wheel("Radio Wheel", catPath, 0, 0, new System.Drawing.Size(Config.IconX, Config.IconY), 200, Config.WheelRadius);

                // Get folders in script's main folder "Custom Radio Stations"
                //string[] stationDirectories = Directory.EnumerateDirectories(wheelDir, "*", SearchOption.TopDirectoryOnly)?.ToArray();

                // Get the stations from the json config
                //string[] stationDirectories = wheelDir.stations.Select(x=>x.name).ToArray();

                Logger.Log("Number of stations: " + wheelDir.stations.Count());
                ...
                foreach (var stationDir in wheelDir.stations)
                {
                    Logger.Log("Loading " + stationDir.name);

                    // Get all files that have the above-mentioned extensions.
                    //var musicFilePaths = Directory
                    //    .EnumerateFiles(stationDir, "*.*", SearchOption.TopDirectoryOnly)
                    ...
                    // Add Wheel Cat
                    WheelCategory wheelCat = new WheelCategory(wheelDir.name, wheelDir.desc);
                    radioWheel.AddCategory(wheelCat);

                    // Add Wheel Cat Item
                    WheelCategoryItem wheelItem = new WheelCategoryItem(stationDir.name, stationDir.desc, stationDir.type);
                    wheelCat.AddItem(wheelItem);

                    // Create station from Wheel cat
                    RadioStation station = null;
                    //new RadioStation(wheelCat, musicFilePaths);

                    switch (stationDir.type)
                    {
                        case "mix":
                            var arr = new string[] { $"/{catPath}/{stationDir.name}/{stationDir.name}.mp3" }; //TODO: hard coded extension
                            station = new RadioStation(wheelCat, arr);
                            break;
                        case "net":
                            station = new RadioStation(wheelCat, new string[] { stationDir.url });
                            break;
                        case "files":
                            station = new RadioStation(wheelCat, stationDir.files, $"/{catPath}/{stationDir.name}/");
                            break;
                        default:
                            break;
                    }
                ...
                if (populatedStationCount > 0)
                {
                    ...
                    radioWheel.CalculateCategoryPlacement(_iconsRadioTxd);                    
                }
            ...
            {
                CitizenFX.Core.UI.Screen.ShowSubtitle("No music found in Custom Radio Stations. " +
        ...
        bool VanillaOrCustomRadioWheelIsVisible()
        {
            ...
            if (Game.IsControlPressed(2, CitizenFX.Core.Control.VehicleRadioWheel) && Game.Player.CanControlCharacter)
            {
