...
namespace SelectorWheel
{
    ...
    public partial class Wheel
    {
        ...
        public string TextureCatBgName { get; private set; }

        Color TextureCatBgColor;
        double TextureCatBgSizeMultiple;

        public long TextureCatBgHandle { get; private set; }

        string TextureCatHlPath = ""; // Highlight path

        public string TextureCatBgHightlightName { get; private set; }

        Color TextureCatBgHighlightColor;
        double TextureCatBgHighlightSizeMultiple;

        public long TextureCatBgHighlightHandle { get; private set; }
        ...
        public void CalculateCategoryPlacement(long txd = 0)
        {
            ...
            {                
                Logger.Log($"texture dict created: radio_bg_icons");
                
                foreach (var cat in Categories)
                {
                    bool hasTexture = false;

                    //create a main category / genre sprite
                    var path = $"{Constants.MAIN_PATH}{cat.Name}/{cat.Name}.png";
                    var h = API.CreateRuntimeTextureFromImage(txd, cat.Name, path);
                    hasTexture = true;

                    //create sprites for the stations
                    foreach (var item in cat.ItemList)
                    {
                        path = $"{Constants.MAIN_PATH}{cat.Name}/{item.Name}.png";                        
                        h = API.CreateRuntimeTextureFromImage(txd, item.Name, path);

                        if (h > 0)
                        {
                            Logger.Log($"creating texture: {path}");
                            item.ItemTexture = new Texture("radio_icons", item.Name, Categories.IndexOf(cat) /*cat.ItemList.IndexOf(item)*/);
                            hasTexture = true;
                        }
                        else
                        {
                            Logger.Log($"creating texture FAIL: {path}");
                        }
                    }                    
                    ...
                    if (hasTexture)
                    {
                        ...
                        {
                            cat.BackgroundTexture = new Texture("radio_bg_icons", TextureCatBgName, Categories.IndexOf(cat) + Categories.Count);
                        }
                        if (!string.IsNullOrWhiteSpace(TextureCatHlPath))
                        {
                            cat.HighlightTexture = new Texture("radio_bg_icons", TextureCatBgHightlightName, Categories.IndexOf(cat) + Categories.Count * 2);
                        }
                    }
        ...
        {            
            ...
            if(_iconsTxd <= 0)
            {
                Logger.Log($"creating texture dict: radio_bg_icons");
                _iconsTxd = API.CreateRuntimeTxd("radio_bg_icons");

                TextureCatBgPath = pathBg;
                TextureCatBgName = "catbg";
                TextureCatBgColor = bgColor;
                TextureCatBgSizeMultiple = bgSizeMultiple;
                TextureCatBgHandle = API.CreateRuntimeTextureFromImage(_iconsTxd, TextureCatBgName, pathBg);

                TextureCatHlPath = pathHl;
                TextureCatBgHightlightName = "cathl";
                TextureCatBgHighlightColor = highlightColor;
                TextureCatBgHighlightSizeMultiple = hlSizeMultiple;
                TextureCatBgHighlightHandle = API.CreateRuntimeTextureFromImage(_iconsTxd, TextureCatBgHightlightName, pathBg);

                API.RequestStreamedTextureDict("radio_bg_icons", false);
                Logger.Log($"texture dict created: radio_bg_icons");
            }            
        }
        ...
        void ControlCategorySelection()
        {
            foreach (var cat in Categories)
            {
                ...
                if (UseTextures && anyTextureExists)
                {
                    ...
                    //    new Point((int)(cat.position2D.X * Screen.Width) + xTextureOffset, (int)(cat.position2D.Y * Screen.Height) + yTextureOffset),
                    ...
                    if (isSelectedCategory && hlTextureExists)
                    {
                        ...
                        //    new Point((int)(cat.position2D.X * CitizenFX.Core.UI.Screen.Width) + xTextureOffset, (int)(cat.position2D.Y * CitizenFX.Core.UI.Screen.Height) + yTextureOffset),
                        //    new PointF(0.5f, 0.5f),
                        //    SizeMultiply(TextureSize, TextureCatBgHighlightSizeMultiple),
                        //    0f, TextureCatBgHighlightColor, UIHelper.AspectRatio);

                        color = TextureCatBgHighlightColor;
                        size = SizeMultiply(new Size(Config.IconX, Config.IconY), TextureCatBgHighlightSizeMultiple);

                        API.DrawSprite("radio_bg_icons", TextureCatBgHightlightName,
                        (cat.position2D.X), (cat.position2D.Y),
                    ...
                    if (bgTextureExists)
                    {
                        ...
                        //    0f,
                        //    isSelectedCategory ? TextureCatBgColor : Color.FromArgb(120, TextureCatBgColor.R, TextureCatBgColor.G, TextureCatBgColor.B),
                        //    UIHelper.AspectRatio);

                        color = isSelectedCategory ? TextureCatBgColor : Color.FromArgb(120, TextureCatBgColor.R, TextureCatBgColor.G, TextureCatBgColor.B);
                        size = SizeMultiply(new Size(Config.IconX, Config.IconY), TextureCatBgSizeMultiple);

                        API.DrawSprite("radio_bg_icons", TextureCatBgName,
                            (cat.position2D.X),
                            (cat.position2D.Y),
                            size.Width / Screen.Width,
                            size.Height / Screen.Height,
                    ...
                    color = isSelectedCategory ? Color.FromArgb(255, 255, 255, 255) : Color.FromArgb(120, 255, 255, 255);
                    size = SizeMultiply(new Size(Config.IconX, Config.IconY), 0.9);

                    //Logger.Log($"cat pos: {cat.position2D.X} x {cat.position2D.Y}");
                    //Logger.Log($"size: {size.Width} x {size.Height}");
                    API.DrawSprite(temp.Txd, temp.Name,
                    (cat.position2D.X),
                    (cat.position2D.Y),
                    size.Width / (Screen.Width + xTextureOffset), size.Height / (Screen.Height + yTextureOffset),
                    //0.03f,0.03f,
                    0,
                    color.R,
                    color.G,
            ...
            {
                float pixelX = 364f / (float)Screen.Width;
                float pixelY = 610f / (float)Screen.Height;
                UIHelper.DrawCustomText(SelectedCategory.SelectedItem.Description, 0.35f, FontDescription, 255, 255, 255, 255, pixelX, pixelY, 0, 0, 0, 0, 0,
                    UIHelper.TextJustification.Left, true, pixelX, 1050f / (float)Screen.Width, true, 0, 0, 0, 180, 10f / (float)Screen.Width, 10f / (float)Screen.Height);
            }
            else if (SelectedCategory.Description != null)
            {
                float pixelX = 364f / (float)Screen.Width;
                float pixelY = 610f / (float)Screen.Height;
                UIHelper.DrawCustomText(SelectedCategory.Description, 0.35f, FontDescription, 255, 255, 255, 255, pixelX, pixelY, 0, 0, 0, 0, 0, 
                    UIHelper.TextJustification.Left, true, pixelX, 1050f / (float)Screen.Width, true, 0, 0, 0, 180, 10f / (float)Screen.Width, 10f / (float)Screen.Height);
            }
        ...
        float InputToAngle()
        {
            var angle = Math.Atan2(Game.GetControlNormal(2, Control.WeaponWheelUpDown), Game.GetControlNormal(2, Control.WeaponWheelLeftRight));
            if (angle < 0)
            {
                angle += Math.PI * 2;
            }
        ...
        {
            return Game.IsControlJustPressed(2, !ControlHelper.UsingGamepad() ?
