...
namespace SelectorWheel
{
    public static class UIHelper
    {
        ...
        public static float MeasureStringWidth(string str, Font font, float fontsize)
        {
            ...
            float ratio = (float)CitizenFX.Core.UI.Screen.Resolution.Width / CitizenFX.Core.UI.Screen.Resolution.Height;
        ...
        {
            return Function.Call<float>(Hash.GET_RENDERED_CHARACTER_HEIGHT, fontSize, (int)font) * CitizenFX.Core.UI.Screen.Resolution.Height; //1080f
        }
        ...
        {
            const float height = 1080f;
            float ratio = (float)Screen.Resolution.Width / Screen.Resolution.Height;
            float width = height * ratio;

            return percent * height;
        }
        
        public static float AspectRatio { get; private set; } = Function.Call<float>(Hash._GET_ASPECT_RATIO, true);

        public static float UpdateAspectRatio()
        {
            AspectRatio = Function.Call<float>(Hash._GET_ASPECT_RATIO, true);
            return AspectRatio;
        }
        ...
        public static PointF PointFromCenter(float x, float y, float normalizedAngle)
        {
            ...
            float x2 = (Screen.Width / 2) + (float)Math.Cos(angle2) * x;
            float y2 = (Screen.Height / 2) + (float)Math.Sin(angle2) * y * (AspectRatio / (16f / 9f));
