...
namespace SelectorWheel
{
    public class Texture
    {
        ...
        public Texture(string txd, string name, int index)
        {
            Txd = txd;
            Name = name;
            Index = index;
        ...
        {
            DrawTexture(filename, index, level, time, pos, new PointF(0.0f, 0.0f), size, 0.0f, Color.FromArgb(255,255,255), 1.0f);

            //API.DrawSprite("", "", center.X, center.Y, w, h, x, y, rotation, aspectRatio, color.R / 255.0f, color.G / 255.0f, color.B / 255.0f, color.A / 255.0f);
        }
        ...
        public static void DrawTexture(string filename, int index, int level, int time, Point pos, PointF center, Size size, float rotation, Color color, float aspectRatio)
        {
            ...
            float x = (float)pos.X / Screen.Width;
            float y = (float)pos.Y / Screen.Height;
            float w = (float)size.Width / Screen.Width;
            float h = (float)size.Height / Screen.Height;
