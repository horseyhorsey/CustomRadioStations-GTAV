...
namespace Settings
{
    public sealed class ScriptSettings
	{
        ...
        /// <summary>
        /// Loads a <see cref="ScriptSettings"/> from the specified file. Cfx LoadResourceFile
        /// </summary>
        /// <param name="filename">The filename to load the settings from.</param>
        public static ScriptSettings Load(string filename)
		{
			Logger.Log("creating script settings for " +  filename);
			var result = new ScriptSettings(filename);

			//load the settings.ini
			var res = API.LoadResourceFile(API.GetCurrentResourceName(), filename);
			if(string.IsNullOrWhiteSpace(res)) return result;
